...
impl InsertFormatted {
    ...
    pub(crate) fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<()>> {
        ...
        // LOCAL PATCH (ushio): a closed channel means the request task already finished, almost
        // always with an error. Surface that error instead of an opaque "channel closed".
        let Some(sender) = self.state.sender() else {
            return self.poll_wait_handle(cx).map(|r| r.and_then(|()| Err(Error::Network("channel closed".into()))));
        };
        ...
        match sender.poll_ready(cx) {
            ...
            Poll::Ready(false) => self.poll_wait_handle(cx).map(|r| r.and_then(|()| Err(Error::Network("channel closed (request completed early)".into())))),
            Poll::Pending => {
